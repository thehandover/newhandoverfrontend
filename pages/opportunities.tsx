                ...
                <div className="flex flex-wrap justify-between items-end py-16 absolute bottom-0 width">
                  <div>
                    ...
                    <p className="text-white text-sm pb-2">
                      {data[0].location?.address}, {data[0].location?.city}{" "}
                      {data[0].location?.zip}
                    </p>
                    <div className="grid grid-cols-4 text-gray-300 text-xs items-center">
                      <span className="flex justify-center">
                        <Image
                          ...
                          alt={"area sq.ft"}
                        />{" "}
                        <span className="ml-1">{data[0].area} sq.ft</span>
                      </span>
                      <span className="flex justify-center">
                        <Image
                          src={BedImage}
                          ...
                          alt={"area sq.ft"}
                        />{" "}
                        <span className="ml-1">{data[0].bedrooms} bed</span>
                      </span>
                      <span className="flex justify-center">
                        <Image
                          src={BedImage}
                          ...
                          alt={"area sq.ft"}
                        />{" "}
                        <span className="ml-1">{data[0].bathrooms} bath</span>
                      </span>
                      <span className="flex justify-center">
                        <Image
                          src={RegreshImage}
                          ...
                          alt={"area sq.ft"}
                        />{" "}
                        <span className="ml-1">3 hours left</span>
                      </span>
                    </div>
                  </div>
                  <div>
                    <Link href={"/opportunity/" + data[0]._id}>
                      <a className="text-black text-sm bg-white font-semibold py-3 px-8 rounded-full block mt-10">
                        View Detail
                      </a>
                    </Link>
                  </div>
          ...
          <div className="content-area w-full">
            <div className="pt-3 ml-0 lg:ml-5 mt-5 lg:mt-0">
              <div className="listing-filters w-full mb-5">
                <h3 className="listing-title text-3xl theme-color font-semibold">
                  All Listings
                </h3>
              </div>
              <div className="py-5">
                <select
                  name=""
                  id=""
                  className="p-2 px-4 text-sm rounded-full border border-solid"
...
export async function getServerSideProps(context: any) {
  ...
  if (
    context.query.minPrice != undefined &&
    context.query.maxPrice != undefined
  ) {
    // for min max filter
    let filter: any = {
  ...
  } else if (context.query.propertyType != undefined) {
    // for text and city and property type filter
    let filter: any = {};
    if (
      context.query.propertyType != undefined &&
      context.query.propertyType != ""
    ) {
      filter.propertyType = context.query.propertyType;
    }
    if (context.query.text != undefined && context.query.text != "") {
      filter.propertyTitle = context.query.text;
    }
    if (context.query.city != undefined && context.query.city != "") {
      filter.location.city = context.query.city;
    }
