...
const Opportunity = ({ property }: any) => {
  return (
    <Link href={`/opportunity/${property._id}`}>
      <a
        className={`relative rounded-lg overflow-hidden ${styles.propertySection}`}
        style={{ height: "16rem" }}
      >
        <Image
        ...
        />
        <div
          className={`absolute bottom-0 top-0 w-full ${styles.propertyContent}`}
          style={{
            background:
              "linear-gradient(0deg, rgba(0, 0, 0,0.8), rgba(0, 0, 0,0.7), rgba(255, 255, 255, 0.2), transparent)",
          }}
        >
          <div className="h-full flex flex-col justify-end px-5 py-3">
            <div className="flex flex-wrap justify-between w-full border-b-2 border-solid border-white pb-2">
              <div style={{ flex: 3 }}>
                <h3 className="text-white text-lg font-semibold pb-1 pr-2">
