...
const PropertyForm = ({
  ...
  const handleSubmitUpdate = async (e) => {
    ...
    let result = await axios({
      method: "PATCH",
      url: `${"https://handoverbackendapp.herokuapp.com"}/property/edit/${
        property._id
      }`,
      headers: {
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 gap-x-10 pb-6 border-solid border-b">
          <div className="grid grid-cols-1 gap-2">
            <label htmlFor="propertyTitle" className="text-gray-500">
              Property title <span className="text-red-500">*</span>
            </label>
            <input
              type="text"
              className="px-3 py-1 rounded-full bg-white border border-solid focus:border-1 focus:border-blue-500 transition-all duration-300"
              id="propertyTitle"
              name="propertyTitle"
          ...
          <div className="grid grid-cols-1 gap-2">
            ...
            <div className="relative overflow-hidden rounded-full border border-solid focus:border-1 focus:border-blue-500">
              <input
                type="text"
                className="px-3 py-1 rounded-full bg-white transition-all duration-300 w-full"
                id="area"
                name="area"
                placeholder="Area in sq.ft"
                value={user.area}
                onChange={(e) => setInNumber(e)}
                required
              />
              <span className="absolute right-0 bottom-0 top-0 bg-gray-50 text-sm text-gray-500 px-2 flex items-center">
                sq.ft
              </span>
            </div>
          </div>
          ...
          {propertyType != "OffPlan" && (
            <>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="grid grid-cols-1 gap-2">
                  <label htmlFor="bathrooms" className="text-gray-500">
              ...
              <div className="grid grid-cols-1 gap-2">
                <label htmlFor="floors" className="text-gray-500">
          ...
          <div className="grid grid-cols-1 gap-2">
            <label htmlFor="priceDemand" className="text-gray-500">
          ...
          <div className="grid grid-cols-1 gap-2">
            <label htmlFor="biddingEnd" className="text-gray-500">
          ...
          <div className="grid grid-cols-1 gap-2">
            <label htmlFor="biddingEnd" className="text-gray-500">
              Est Completion <span className="text-red-500">*</span>
            </label>
            <input
              type="text"
              className="px-3 py-1 rounded-full bg-white border border-solid focus:border-1 focus:border-blue-500 transition-all duration-300"
              id="estCompletion"
              name="estCompletion"
              placeholder="e.g. __ years | __ months | __ days"
              required
            />
          </div>
          ...
          <div className="grid grid-cols-1 gap-2">
            <label htmlFor="type" className="text-gray-500">
              Select payment plan <span className="text-red-500">*</span>
            </label>
            <select
              className="px-3 py-1 rounded-full bg-white border border-solid focus:border-1 focus:border-blue-500 transition-all duration-300"
              id="type"
          ...
          <div className="grid grid-cols-1 gap-2 col-span-2">
            ...
            <Editor
              ...
              onEditorChange={(text) => setUser({ ...user, description: text })}
            />
          </div>
          ...
          <div className="grid grid-cols-1 gap-2 col-span-2">
            <label
              htmlFor="images"
              className="text-black font-medium border border-solid border-gray-500 px-10 py-1 w-max rounded-full cursor-pointer"
            >
              Upload property images
            </label>
            <input
              type="file"
              className="hidden px-3 py-1 rounded-full bg-white border border-solid focus:border-1 focus:border-blue-500 transition-all duration-300"
              id="images"
              name="images"
              multiple
            ...
            <div className="grid grid-cols-1 gap-2 col-span-2">
              <div className="flex flex-wrap">
                {Array.from(multiImages).map((link, i) => {
                  return (
                    <div
                      className="relative border border-solid mr-2 rounded"
          ...
          <div className="pb-6 border-solid border-b shadow mt-4 rounded-xl overflow-hidden col-span-2">
            <div className="p-4 bg-gray-900 w-full text-white">
              <label htmlFor="">
                Amineties <small className="text-gray-100">(optional)</small>
              </label>
            </div>
                  ...
                  <div key={index}>
                    <div className="flex pb-2">
                      <input
                        ...
                        id="aminity"
                        name="aminity"
                        placeholder="Amineties"
                        value={aminity.aminity}
                        style={{ flex: 8 }}
                        onChange={(e) => changeAminityValue(e, index)}
                      />
                      {index > 0 && (
                        <button
                          type="button"
                          className="btn bg-red-500 w-max px-4 py-1 text-white rounded-full ml-2"
                          onClick={() => removeNewAnimity(index)}
                          style={{ flex: 1 }}
                        >
                          Remove
                        </button>
                    ...
                    {aminitiesList.length - 1 === index && (
                      <div className="mt-2">
                        <button
                          type="button"
                          className="btn bg-gray-900 w-max px-4 py-1 text-white rounded-full"
                          onClick={() => addNewAnimity()}
                        >
                          Add New
                        </button>
                      </div>
        ...
        <div className="mt-4 flex">
          {property ? (
            <>
              <button
                className="btn bg-black text-white rounded-full px-4 py-1 mr-4 flex"
              ...
              <button
                type="button"
                className="btn bg-red-100 hover:bg-red-500 text-red-500 hover:text-white rounded-full px-4 py-1 mr-4 transition-all duration-300"
            ...
            <>
              <button
                className="btn bg-black text-white rounded-full px-4 py-1 mr-4 flex flex-wrap"
                disabled={loading ? true : false}
              >
                Save{" "}
                {loading && (
              ...
              <Link href={"/seller/properties"}>
                <a className="btn bg-red-100 hover:bg-red-500 text-red-500 hover:text-white rounded-full px-4 py-1 mr-4 transition-all duration-300">
                  Cancel
                </a>
              </Link>
            </>
